...
import sys
#print(sys.path)  # Debugging: Check if bleak's location is listed
...
def parse_accel_data(data, device_index, device_name):
    ...
    try:
        if device_name in  ESP32_NAMES :
            num_values = len(data) // 4  # Each float is 4 bytes
...
def start_collection():
    global patient_name, stop_event, start_time, ESP32_ADDRESSES
    #stop_event = threading.Event()
    stop_event.clear()
...
def create_gui():
    ...
    tk.Label(root, text="Button Order: 1. Discover Devices, 2. Select the devices, 3. Save Inputs, 3. Start, 4. Stop").grid(row=21, column=0)
    root.mainloop()

...
ICM42670 IMU(Wire, 0);
volatile bool startTransfer = false;  // Flag for Deadlock fix
volatile bool resetCycle = false;    // Flag for Automation
...
//        Serial.print("BLE received: ");
        ...
        // --- Handle the ACK from the client ---
        else if (value == "ACK") {
...
void loop() {
  // 1. Handle File Transfer (Deadlock Fix)
  if (startTransfer) {
    startTransfer = false;
    exportFileOverBLE();   
  }

  // 2. Handle Cycle Reset (Automation)
  if (resetCycle) {
    resetCycle = false;
    Serial.println("Resetting Cycle...");
    SPIFFS.remove(filename); // Wipe memory
    startLogging();          // Restart logging immediately
    
    // Notify the App that we are back in business
    pCharacteristic->setValue("LOGGING_STARTED");
...
void logIMUDataToFile() {
  if (SPIFFS.usedBytes() > (SPIFFS.totalBytes() * 0.02)) {
    Serial.println("MEMORY FULL - Requesting Upload");
    stopLogging(); 

    // Send "FULL" signal to the App to trigger download
    pCharacteristic->setValue("FULL"); 
    pCharacteristic->notify(); 
    return;
  }

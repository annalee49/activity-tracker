...
volatile bool ackReceived = false; // NEW: Tracks if JS sent the "ACK" signal
...
class MyCallbacks: public BLECharacteristicCallbacks {
  ...
  if (value.length() > 0) {
    ...
    // --- NEW: ACK HANDLER ---
        ...
        ackReceived = true;
        // Serial.println("ACK Received"); // Optional debug
    }
  }
};
...
void exportFileOverBLE() {
    ...
    int packetCounter = 0; // Tracks packets in the current burst
    ...
    while(file.available()) {
        ...
        if (bytesRead > 0) {
            ...
            // 3. Check if we reached the burst limit (5 packets)
            // This MUST match the "_ackCounter >= 5" in your HTML
            if (packetCounter >= 5) {
                
                ackReceived = false; // Reset flag before waiting
                
                unsigned long startTime = millis();
                
                // WAIT LOOP: Pause until ACK received or 1.5s timeout
                while (!ackReceived && (millis() - startTime < 1500)) {
                    delay(1); // Yield to system tasks
                }

                if (!ackReceived) {
                    Serial.println("ACK Timeout! Continuing anyway...");
                }
                
                packetCounter = 0; // Reset counter for next burst
            } else {
                // Small delay between packets within a burst 
                // to prevent local stack congestion
                delay(5); 
            }
        }
    }
...
void logIMUDataToFile() {
  ...
  LogEntry currentLog;
  currentLog.timestamp = millis() + timeOffset; // Now matches real world time

...
// --- states ---
enum DeviceState { STATE_LOGGING, STATE_WAITING_TO_TRANSFER };
DeviceState currentState = STATE_LOGGING;
...
class MyCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
      ...
      if (value == "export_ble") {
        if (currentState == STATE_WAITING_TO_TRANSFER) {
          // 1. Export the file
          exportFileOverBLE();

          // 2. Clear the file (This is a new step!)
          Serial.println("Transfer complete. Clearing file.");
          SPIFFS.remove(filename);

          // 3. Go back to logging
          Serial.println("Restarting logging.");
          currentState = STATE_LOGGING;
          startLogging();
          
        } else {
          Serial.println("Received 'export_ble' but not ready to transfer.");
        }
      }
    }
};
...
void loop() {
  if (currentState == STATE_LOGGING) {
    ...
    if (isLogging && (millis() - lastLogTime >= logInterval)) {
      lastLogTime = millis();
      logIMUDataToFile(); // [Source 100] This function can now change our state
    }
    
  } else if (currentState == STATE_WAITING_TO_TRANSFER) {
    // In this state, we are NOT logging.
    // We are just waiting for a phone to connect and send the
    // "export_ble" command [Source 85] (or a new "confirm_transfer" command).
    
    // --- (BLE connection/command logic is handled by callbacks) ---
  }
}
...
void logIMUDataToFile() {
  ...
  if (SPIFFS.usedBytes() > (SPIFFS.totalBytes() * 0.98)) {
    Serial.println("!!! MEMORY FULL - PREPARING FOR TRANSFER !!!");
    stopLogging(); // [Source 130]
    currentState = STATE_WAITING_TO_TRANSFER;
    
    // Start advertising (or send a notification if already connected)
    // This tells the app: "I am full. Please connect and download."
    BLEDevice::startAdvertising(); // [Source 96]
    
    // You could also send a one-time message
    pCharacteristic->setValue("FILE_READY");

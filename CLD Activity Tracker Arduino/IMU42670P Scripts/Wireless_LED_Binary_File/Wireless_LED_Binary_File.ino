...
// --- NEW: WiFi & Web Server Settings ---
const char* ssid = "IMU_LOGGER_AP"; // The WiFi network name
WebServer server(80);                // The web server object on port 80
// --- END NEW ---
...
void setup() {
  ...
  // --- NEW: Start WiFi Access Point (AP) ---
  ...
  Serial.println(apIP); // This is the address you'll type in your browser

  // --- NEW: Define server pages ---
  server.on("/", HTTP_GET, handleRoot);           // Home page
  server.on("/data", HTTP_GET, handleDownload); // Download link
...
// --- NEW: Web Server Handler Functions ---

// This function runs when you first connect in your browser
void handleRoot() {
  ...
  if (!SPIFFS.exists(filename)) {
    server.send(200, "text/html", 
      "<html><body>"
      "<h1>IMU Logger</h1>"
      "<p>No data file found. Please log some data first.</p>"
      "</body></html>");
    return;
  }
  ...
  html += "<h1>IMU Logger</h1>";
  html += "<p>Data file found (" + String(fileSize) + " bytes).</p>";
  html += "<p>Click the link to download the binary data file.</p>";
  html += "<h2><a href='/data'>Download imu_data.csv</a></h2>";
  html += "</body></html>";
  
  server.send(200, "text/html", html);
}

// This function runs when you click the "Download" link
void handleDownload() {
  File file = SPIFFS.open(filename, FILE_READ);
  if (!file) {
  ...
  // This header tells the browser to treat it as a download
  server.sendHeader("Content-Disposition", "attachment; filename=imu_data.csv");
  server.sendHeader("Content-Type", "application/octet-stream"); // This is the key
